...
// This method and the one below work together
...
list_loadconf (char *file, GSList ** list, char *defaultconf)
{
	...
	// Creates a file read. (filename, flags, mode) on POSIX / Non Windows
	fd = g_open (filebuf, O_RDONLY | OFLAGS, 0);
	g_free (filebuf);

	if (fd == -1)
	{
		// defaultconfig is a char pointer with default menu
		if (defaultconf)
...
load_default_config(void)
{
	...
	// Default value for hexchat settings

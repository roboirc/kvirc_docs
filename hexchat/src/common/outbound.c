...
// Echo command in HexChat just calls PrintText
static int
...
// Edited 9/4/2019
static int cmd_test (struct session *sess, char *tbuf, char *word[], char *word_eol[]) {

	// Can print text by defining custom variables too
	#define Welcome "\nWelcome to HexChat."

	PrintText(sess, Welcome);
	PrintText(sess, "\nWelcome to cmd_test().");

	// word_eol[2] gives the full text after the command
	// word_eol[1] gives the full text including command
	PrintText(sess, "word_eol[1] is: ");
	PrintText(sess, word_eol[1]);

	// word[] gives the specific word in the command
	PrintText(sess, "word[3] is: ");
	PrintText(sess, word[3]);

	/* Define a temporary variable */
    char example[100] = "";

    /* Copy the first string into the variable */
    // strcpy(example, "Welcome to cmd_test().");

    /* Concatenate the following two strings to the end of the first one */
    strcat(example, "roboirc's test command!. ");
    strcat(example, "This was done using string.h (C's strcat library). ");
	PrintText(sess, example);
...
cmd_help (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	// If the -l flag was chosen, show long format help
	if (*helpcmd && strcmp (helpcmd, "-l") == 0)

		longfmt = 1;

	// If the -l flag was not chosen, show short help
	if (*helpcmd && !longfmt)
	{
...
{
	// Extract the channel name as the second argument
	char *chan = word[2];
	session *sess_find;

	// If a channel name is given, argument 2
	if (*chan)
	{
		// Password is argument 3
		char *po, *pass = word[3];

		// Look for the channel on server
		...
		if (!sess_find)
		{
			...
			{
				//Goes through list of channels if more than 1
				po = strchr (chan, ',');
...
{
	// Person's nickname is 2nd argument
	char *nick = word[2];
...
cmd_list (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	PrintText (sess, "Listing Channels ...");

    // Opens a window with channel listing
...
cmd_part (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	// If channel name is not specified, its the current 'window' session's channel
	if (!*chan)
		chan = sess->channel;

	// Logical AND operation. If channel is given and it exists
	if ((*chan) && is_channel (sess->server, chan))
	{
		// If reason is not given, make it NULL else send reason
		if (reason[0] == 0)
			reason = NULL;

		// This line sends the part command to the server in irc
		server_sendpart (sess->server, chan, reason);

		// This closes the channel that was parted
		fe_close_window(sess);
...
// Associates the command with the method that is called
// The three binary values correspond to whether the command is an offline or online command
// They correspond to char needserver; char needchannel; gint16 handle_quotes;
// This is a struct array of commands
// Has 6 elements {char *name, cmd_callback callback, char needserver, char needchannel,
// gint16 handle_quotes, char *help}
...
const struct commands xc_cmds[] = {
	...
	{"SPLAY", cmd_splay, 0, 0, 1, "SPLAY <soundfile>"},	{"TEST", cmd_test, 0, 0, 1, N_("roboirc's test command")},
	{"TOPIC", cmd_topic, 1, 1, 1, N_("TOPIC [<topic>], sets the topic if one is given, else shows the current topic")},
	{"TRAY", cmd_tray, 0, 0, 1,
	...
	{"UNQUIET", cmd_unquiet, 1, 1, 1, N_("UNQUIET <mask> [<mask>...], unquiets the specified masks if supported by the server.")},
	{"URL", cmd_url, 0, 0, 1, N_("URL <url>, opens a URL in your browser")},
	{"USELECT", cmd_uselect, 0, 1, 0, N_("USELECT [-a] [-s] <nick1> <nick2> etc, highlights nick(s) in channel userlist")},
	{"USERLIST", cmd_userlist, 1, 1, 1, 0},
	{"VOICE", cmd_voice, 1, 1, 1, N_("VOICE <nick>, gives voice status to someone (needs chanop)")},
	{"WALLCHAN", cmd_wallchan, 1, 1, 1, N_("WALLCHAN <message>, writes the message to all channels")},
	{"WALLCHOP", cmd_wallchop, 1, 1, 1, N_("WALLCHOP <message>, sends the message to all chanops on the current channel")},

...
// This is the inputbox call back
void
mg_inputbox_cb (GtkWidget *igad, session_gui *gui)
{
	...
	// This is the contents of what is entered into the inputbox
...
// This displays the title of the session whether its a channel,
// dialog or network window
void
fe_set_title (session *sess)
{
	...
	// Checks what type of session it is
	type = sess->type;

	// Default to simple window title
	if (sess->server->connected == FALSE && sess->type != SESS_DIALOG)
		goto def;

	// Checks what type of session it is and sets the title of window
	switch (type)
	{
	case SESS_DIALOG:
	...
	case SESS_CHANNEL:
		...
		// Displays channel modes if enabled
		...
		// In this section, the total number of users is shown in the topic
	...
	// Finally the window title is set
	gtk_window_set_title (GTK_WINDOW (sess->gui->window), tbuf);
}
...
mg_set_topic_tip (session *sess)
{
	...
	case SESS_CHANNEL:
		if (sess->topic)
		{
			text = g_strdup_printf (("%s : %s"), sess->channel, sess->topic);
			//text = g_strdup_printf (_("Topic for %s is: %s"), sess->channel, sess->topic);
...
mg_tab_close (session *sess)
{
	...
	else
	{
		...
		dialog = gtk_message_dialog_new (GTK_WINDOW (parent_window), 0,
						...
						_("This server has %d channels or dialogs open. "
						  "Close them all?"), i);
		...
		// 18/4/2019
		// Disabled options
		// Made any dialog to show in center of application by default
...
mg_open_quit_dialog (gboolean minimize_button)
{
	...
	text = g_strdup_printf ("<span weight=\"bold\" color=\"red\" size=\"larger\">%s</span>\n\n%s\n%s",
	...
	//Selects the current button selected in the dialog
...
mg_submenu (GtkWidget *menu, char *text)
{
	...
	// Disabled 16/4/2019
	// Right click menu in inputbox
	// These lines append the custom menu to the right click shell
	//gtk_menu_shell_append (GTK_MENU_SHELL (menu), item);
	//gtk_widget_show (item);

	// To add a separator in menu
...
// current_sess contains properties of the current session / channel
// Null prints the text in the current open window
// PrintText (NULL, current_sess->channel);
// g_snprintf is often used in the code because it provides a limit on its size and allows
// formatting string along with extra arguments

char tbuf[256];
if (current_sess)
{
	    g_snprintf (tbuf, sizeof tbuf, "CYCLE %s", current_sess->channel);
...
mg_create_perchannelmenu (session *sess, GtkWidget *menu)
{
	...
	// Added 5/4/2019
	// sess struct can be expanded to include more options / settings
	mg_perchan_menu_item (_("Settings Test"), submenu, &sess->alert_beep, prefs.hex_input_beep_chans);

	submenu = menu_quick_sub (_("_Test"), menu, NULL, XCMENU_MNEMONIC, -1);
	mg_perchan_menu_item (_("Test 1"), submenu, &sess->test1, prefs.hex_irc_logging);
	mg_perchan_menu_item (_("Test 2"), submenu, &sess->test2, prefs.hex_irc_logging);
	mg_perchan_menu_item (_("Test 3"), submenu, &sess->test3, prefs.hex_irc_logging);
...
mg_create_alertmenu (session *sess, GtkWidget *menu)
{
	...
	// Added 2/4/2019
	// Testing
	mg_perchan_menu_item (_("Alert Test"), submenu, &sess->alert_beep, prefs.hex_input_beep_chans);
}

// 27/4/2019
// Can close all open queries in the current channel in this function
void mg_close_all_queries () {

	PrintText (sessquery, "\nmg_close_all_queries");
	...
	for (i = 0, list = sess_list; list; list = list->next)
		{
			...
			// This is how to print anything by writing it to a buffer via g_snprintf
			char buf[100];
			int item;
			g_snprintf (buf, sizeof (buf), "Current item type is: ");
			item = s->type;
			if(item==1)
			strcat(buf, "SESS_SERVER");
			if(item==2)
			strcat(buf, "SESS_CHANNEL");
			if(item==3)
			strcat(buf, "SESS_DIALOG");
			if(item==4)
			strcat(buf, "SESS_NOTICES");
			if(item==5)
			strcat(buf, "SESS_SNOTICES");

			PrintText(sessquery, buf);
...
// Creates the right click menu in channel list
static void
mg_create_tabmenu (session *sess, GdkEventButton *event, chan *ch)
{
	//Declaring the menu widget
	GtkWidget *menu, *item;
	char buf[256];
	...
	if (sess)
	{
		// This is the channel's name on which right click has been pressed
		char *name = g_markup_escape_text (sess->channel[0] ? sess->channel : _("<none>"), -1);

		//g_snprintf (buf, sizeof (buf), "<span foreground=\"#3344cc\"><b>%s</b></span>", temp);
		...
		if (sess->type == SESS_CHANNEL) {
			...
			// Shows the Autojoin option
			menu_addfavoritemenu (sess->server, menu, sess->channel, TRUE);

			// Channel rejoin / cycle
			mg_create_icon_item (_("_Cycle Channel"), GTK_STOCK_CONNECT, menu, mg_cycle, NULL);

		}

		else if (sess->type == SESS_SERVER) {

			// Created a menu to close all open queries for the current server
			sessquery = sess;
			mg_create_icon_item (_("Close all queries"), GTK_STOCK_CLOSE, menu, mg_close_all_queries, ch);

			// Shows the Autoconnect option
			menu_addconnectmenu (sess->server, menu);
		}
	}

	// Creates the Detach/Attach item
...
// This function deals with text highlighting / selection by left mouse
// click button
static void
mg_word_clicked (GtkWidget *xtext, char *word, GdkEventButton *event)
{
	session *sess = current_sess;
	...
	{
		// If nothing is selected, return focus to the session
		if (word == NULL)
		{
			mg_focus (sess);
			return;
	...
	// Testing
	// 19/4/2019
	//if ((event->button == 1) && (sess->type == SESS_CHANNEL)){

	//	PrintText (NULL, "mouse was single clicked in a channel window!");
	//}
	...
	// Checks what type of word was selected and opens the
	...
	case WORD_HOST:
		word[end] = 0;
		word += start;
		menu_urlmenu (event, word);
		break;
	case WORD_NICK:
		word[end] = 0;
		word += start;
		menu_nickmenu (sess, event, word, FALSE);
		break;
	case WORD_CHANNEL:
		word[end] = 0;
		word += start;
		menu_chanmenu (sess, event, word);
		break;
	case WORD_EMAIL:
		word[end] = 0;
		word += start;
		...
		// 1/7/2019
		// Removed "Email :"
		tmp = g_strdup_printf ("%s", word + (ispunct (*word) ? 1 : 0));
		menu_urlmenu (event, tmp);
		g_free (tmp);
		break;
	case WORD_DIALOG:
		menu_nickmenu (sess, event, sess->channel, FALSE);
		break;
	}
}
...
mg_create_entry (session *sess, GtkWidget *box)
{
	...
	//Disabled 16/4/2019
	//g_signal_connect (G_OBJECT (but), "clicked", G_CALLBACK (mg_nickclick_cb), NULL);
	...
	// entry is the inputbox and is connected to activate signal here
	g_signal_connect (G_OBJECT (entry), "activate",
							G_CALLBACK (mg_inputbox_cb), gui);

	// entry is added to the hbox container

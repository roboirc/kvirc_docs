...
// This is the struct for each menu item
struct mymenu
{
	char *text;					// Name of menu item
	void *callback;				// Callback function
	char *image;
	unsigned char type;	        // M_XXX / Menu Item Type
	unsigned char id;		    // MENU_ID_XXX (menu.h)
	unsigned char state;	    // Ticked or not?
	unsigned char sensitive;    // Shaded out?
	guint key;	            	// GDK_KEY_x / Shortcut Key
};

// Functions for simple test menu function popup
void test1 ()
{
	PrintText(NULL, "test1");
...
void user_op()
{
	message("testing menu code");
}

/* returns boolean: Some data is missing */

// This is the nicklist right click popup menu on the right side
// submenu is a gtkwidget which is a menu onto which items can be added
static gboolean
menu_create_nickinfo_menu (struct User *user, GtkWidget *submenu)
{
	...
	// String format as argument
	// This is the bold formatting string in g_snprintf function
	// The number decides how many characters will be bold / underlined / etc
	// negative(-) means starting from left side
	fmt = _("<tt><b>%-15s</b></tt> %s");

	// This is where unknown is declared and formatted
	g_snprintf (unknown, sizeof (unknown), "<i>%s</i>", _("Unknown"));

	//item = menu_quick_item (0, "Nickname Details", submenu, XCMENU_MARKUP, 0, 0);
	...
	if(user->op==1) {
		item = menu_quick_item (0, "User is an op", submenu, XCMENU_MARKUP, 0, 0);
		//g_signal_connect (G_OBJECT (item), "activate", G_CALLBACK (user_op), "user->op==1");
		//strcat(temp, user->nick);
		//strcat(temp, " is an op: User->op==1");
		//PrintText(sess, temp);
		}
	if(user->hop==1) {
		item = menu_quick_item (0, "User is a half op", submenu, XCMENU_MARKUP, 0, 0);
		//g_signal_connect (G_OBJECT (item), "activate", G_CALLBACK (user_op), "user->hop==1");
		//strcat(temp, user->nick);
		//strcat(temp, " is a half op: User->hop==1");
		//PrintText(sess, temp);
		}
	if(user->voice==1) {
		item = menu_quick_item (0, "User is a voice", submenu, XCMENU_MARKUP, 0, 0);
		//g_signal_connect (G_OBJECT (item), "activate", G_CALLBACK (user_op), "user->voice==1");
		//strcat(temp, user->nick);
		//strcat(temp, " is a voice: User->voice==1");
		//PrintText(sess, temp);
		}
	...
	if (user->realname)
	{
		...
		g_snprintf (buf, sizeof (buf), fmt, _("Real Name:"), user->realname);
		//g_free (real);
	} else
	{
...
// Checkout this function for multiple selection related code
void
...
// This is the right click menu over a url
void
menu_urlmenu (GdkEventButton *event, char *url)
{
	...
	// Two options from right click menu when over a URL
	...
	menu_quick_item_with_callback (copy_to_clipboard_cb, _("Copy"), menu, str_copy);

	// Custom entries are kept in urlhandlers.conf
...
// Cycle is equivalent to sending a PART then a JOIN command.
static void
...
// This is the right click menu when hovering over a channel name in a channel
void
...
menu_about (GtkWidget *wid, gpointer sess)
{
	...
	gtk_about_dialog_set_copyright (dialog, "[RoboIRC]");
...
// This is the full menu for the main HexChat program. It is a struct array
// This is also the full menu that is shown when right click in a window happens
// There are 8 elements per struct.
// Each of the fields in each element of the struct define a flags
// The fourth element in the array identifies the type of menu
// Each menu item's call back function is the second element if it exists
// Most structs have 7 elements but if they have 8 it means that there is a shortcut key combination for the option
// defined by a GDK_KEY identifier (eg CTRL + T). Then that shortcut is appended in the name also (tested).
// Notice the line offsets defined for use in code.
// Offsets are used internally to jump to menu item via program
...
#define AWAY_OFFSET (41)
	...
	// Custom menu
	// 11/4/2019
	{N_("_Test"), 0, 0, M_NEWMENU, 0, 0, 1},
	{N_("_Test1"), test1, 0, M_MENUITEM, 0, 0, 1},
	{N_("_Test2"), test2, 0, M_MENUITEM, 0, 0, 1},
						...
						GtkWidget **menu_widgets)
{
	...
	// The offset is used to keep track of indices in the struct array
	// to change the text once window is attached and then detached
	if (!toplevel)
	{
	...
	while (1)
	{
		...
		// This section of code identifies what type of menu item is
		// and deals with it appropriately via switch case
...
normalitem:
			...
			// This checks the last element in the mymenu struct
			if (mymenu[i].key != 0)

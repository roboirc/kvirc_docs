...
// Added 30/3/2019
// Simple message procedure
void message(char *msg){

	GtkWidget *dialog;


	dialog = gtk_message_dialog_new (GTK_WINDOW (parent_window), 0, GTK_MESSAGE_INFO, GTK_BUTTONS_OK, "%s", msg);

	// The response signal is connected to gtk_widget_destroy method
	g_signal_connect (G_OBJECT (dialog), "response",
							G_CALLBACK (gtk_widget_destroy), 0);


	gtk_window_set_title(GTK_WINDOW (dialog), "Welcome to HexChat!");
	gtk_window_set_resizable (GTK_WINDOW (dialog), FALSE);

	// This will set the position of the window same as the location of the mouse pointer
